...
func (r *SQLiteRepository) CreateActivity(activity models.Activity) (*models.Activity, error) {
	res, err := r.db.Exec("INSERT INTO Activity(OldDate, NewDate, Paid, Approved, UserID, CreationDate, CreationUserID, UpdateDate, UpdateUserID) values(?,?,?,?,?,?,?,?,?)",
		activity.OldDate, activity.NewDate, activity.Paid, activity.Approved, getUserId(), time.Now(), nil, nil)
	if err != nil {
...
func (r *SQLiteRepository) All() ([]models.Activity, error) {
	rows, err := r.db.Query("SELECT * FROM Activitys")
	if err != nil {
		return nil, err
	}
	defer rows.Close()
